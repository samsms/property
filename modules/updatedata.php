...
//load  table data
            ...
            $("#tabs-3").load("loadpropertydetails.php?table=floorplan", function (e) {
                                ...
                                $(this).on("click", function (e) {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (data) {
                                            if (data.status == true) {
                                                alert('deleted');
                                                $("#adddeposit").dialog("close")}}, error: function (data) {
                                            if (data.status != true) {
                ...
                $(".editfloor").each(function (e) {
                    $('body').on("click", ".editfloor", function (e) {
                        ...
                        $("#floornumber").val($("#floornumbertd" + id).html());
                        $("#housenumber").val($("#housenumbertd" + id).html());
                        $("#monthrent").val($("#monthrenttd" + id).html());
                        $("#mktvalue").val($("#marketvaluetd" + id).html());
                        $("#elecmeter").val($("#elecmetertd" + id).html());
                        $("#watermeter").val($("#watermetertd" + id).html());
                        $("#metereading").val($("#metereadingtd" + id).html());
                        $("#receipt_due").val($("#receipt_duetd" + id).html())
                ...
                $(".disablefloor").each(function (e) {
                    $('body').on("click", ".disablefloor", function (e) {
                        id = $(this).attr("id");
                        $(function () {
                            $("#disablefloor").dialog({
                                resizable: false,
        ...
        function addFloorplan() {
            if ($("#floornumberadd").val() == '' || $("#housenumberadd").val() == '' || $("#monthrentadd").val() == '' || $("#mktvalueadd").val() == '' || $("#elecmeteradd").val() == '' || $("#watermeteradd").val() == '' || $("#metereadingadd").val() == '') {
                alert("please supply all details");
                return false;
            }
            var myData;
            $.ajax({
        ...
        //disable
            function disableFloorplan() {
            var myData;
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "callfunctions.php?disable=true&apt_id=" + id,
                data: myData,
                success: function (data) {
                    if (data.status == true) {
                        alert('disable');
        ...
        function addChargeables() {
            if ($("#itemadd").val() == '' || $("#chargeamountadd").val() == '') {
                ...
                if ($("#chargeitemvat:checkbox:checked").length > 0) {
                    vat = 1;
                }
                if ($("#chargeitemcommission:checkbox:checked").length > 0) {
                    commission = 1;
                }
                if ($("#chargeitemdeposit:checkbox:checked").length > 0) {
                    isdeposit = 1;
                }
                var myData;
                $.ajax({
...
<div id="disablefloor" title="Confirm Delete" style="display:none;">Do you want to disable floor?  </div>
<div id="deletefloor" title="Confirm Disable" style="display:none;">Do you want to delete record?  </div>
