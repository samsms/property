    ...
    if ($_REQUEST['table'] === 'floorplan') {
        ...
        <table id="treport" style="width:100%" class="display dataTable"><br>
            <thead><tr><td colspan="13" style="text-align:end">
                ...
                <tr><th>No</th><th>Floor</th><th>House No</th><th>Rent/month</th><th>Mkt Value</th><th>Annual rent</th><th>Elec Meter</th>
                    <th>Water Meter</th><th>Meter Reading</th><th>Deposit</th><th>Receipt Due</th><th>Edit</th><th>Disable</th><th>Delete</th></tr>
            </thead><tbody>
                <?php
                $count = 1;
                foreach ($rows as $row) {
                    ...
                    <tr class="floorplanrow" id="<?php echo $row['apt_id'] ?>">
                        ...
                        <td id="floornumbertd<?php echo $row['apt_id'] ?>">
                            <?php echo $row['floornumber'] ?></td>
                        <td id="housenumbertd<?php echo $row['apt_id'] ?>"> 
                            <?php echo $row['apt-tag'] ?></td>
                        <td id="monthrenttd<?php echo $row['apt_id'] ?>">
                            <?php echo $row['monthlyincome'] ?></td>
                        <td id="marketvaluetd<?php echo $row['apt_id'] ?>">
                            <?php echo $row['marketvalue'] ?></td>
                        <td><?php echo $row['yearlyincome'] ?></td>
                        <td id="elecmetertd<?php echo $row['apt_id'] ?>">
                            <?php echo $row['elecmeter'] ?></td>
                        <td id="watermetertd<?php echo $row['apt_id'] ?>">
                            <?php echo $row['watermeter'] ?></td>
                        <td id="metereadingtd<?php echo $row['apt_id'] ?>">
                            <?php echo $row['metereading'] ?></td>
                        <td><a style="color:blue" href="#" id="<?php echo $row["apt_id"] ?>" class="deposits">deposits</a></td>
                        <td id="receipt_duetd<?php echo $row['apt_id'] ?>"><?php echo $row['receipt_due'] ?></td>
                        <td><?php if ($_SESSION['usergroup'] == 1) { ?>
                            <a href="#" id="<?php echo $row["apt_id"] ?>" class="editfloor">
                                <img alt="edit" src="../images/edit.png"></a><?php } ?></td>
                       <td>      <?php if ($_SESSION['usergroup'] == 1) { ?>
                                <a href="#" id="<?php echo $row["apt_id"] ?>" class="disablefloor">
                                    <img alt="del" src="../images/del.png"> </a><?php } ?></td>
                        <td>
                                    <?php if ($_SESSION['usergroup'] == 1) { ?>
                                <a href="#" id="<?php echo $row["apt_id"] ?>" class="deletefloor">
                                    <img alt="del" src="../images/del.png"> </a><?php } ?></td>
