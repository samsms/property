...
$payments = $conn->query("SELECT * from payments where loan_id = 14");
...
$next = $conn->query("SELECT * FROM loan_schedules where loan_id = '14'  order by date(date_due) asc limit 1  $offset")->fetch_assoc()['date_due'];
$sum_paid = 0;
...
if(is_null($next)){
    ...
    $loan=json_encode(array("success"=>true,"month"=>$next,"amount"=>""));
    echo $loan;
}
